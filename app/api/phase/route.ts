...
export async function POST(request: NextRequest) {
	...
	const data = await supabase.from("phase").insert(body);
...
export async function PUT(request: NextRequest) {
	...
	const body = await request.json();
	const supabase = createClient(cookies());
	const data = await supabase.from("phase").update(body).eq("id", Number(id));
	return NextResponse.json({ success: data.error == null, data });
}
...
export async function DELETE(request: NextRequest) {
	...
	const id = request.nextUrl.searchParams.get("id");
	const supabase = createClient(cookies());
	const data = await supabase.from("phase").delete().eq("id", Number(id));
	return NextResponse.json({ success: data.error == null, data });
}
